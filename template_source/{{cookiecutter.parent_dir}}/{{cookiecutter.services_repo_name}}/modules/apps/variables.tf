locals {
  ...
  {{cookiecutter.service_name_underscored}}_service = var.services["{{cookiecutter.service_name_underscored}}"]
  ...
  vpn_cidr_blocks = [TO_FILL]{% if cookiecutter.is_sqs == "true" %}
  ...
  secrets_recovery_window_days = var.is_dr ? 0 : 30{% endif %}{% if cookiecutter.is_es == "true" %}
...
}{% endif %}{% if cookiecutter.is_apigw == "true" %}
